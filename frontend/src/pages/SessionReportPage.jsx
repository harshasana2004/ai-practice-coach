...
// A reusable card component for consistent styling on the report page
const ReportCard = ({ title, children, color = 'blue' }) => {
    // Determine title color based on the prop
    const titleStyle = {
        fontSize: '1.125rem',
        fontWeight: '600',
        marginBottom: '0.75rem',
        color: `var(--color-${color}-700, #1d4ed8)` // Default to blue
    };

    // Define colors for use in the style attribute
    const colors = {
        blue: { '700': '#1d4ed8' },
        purple: { '700': '#7e22ce' },
        gray: { '700': '#374151' },
    ...
    titleStyle.color = colors[color]['700'];
...
const SessionReportPage = () => {
  ...
  if (loading) {
    return (
      <div className="app-container">
        <Header />
        <p style={{ paddingTop: '5rem', textAlign: 'center' }}>Loading report...</p>
      </div>
    );
  }
  ...
  if (!session) {
    return (
      <div className="app-container">
        <Header />
        <p style={{ paddingTop: '5rem', textAlign: 'center' }}>Session not found.</p>
        <div style={{ textAlign: 'center', marginTop: '1rem' }}>
          <Link to="/history" style={{ color: '#4f46e5', textDecoration: 'underline' }}>← Back to History</Link>
        </div>
      </div>
    );
  }
        ...
        <div className="content-wrapper" style={{ maxWidth: '56rem' }}>
            <div className="report-page">
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                    {session.audioURL && (
                        <ReportCard title="Your Recording" color="purple">
                            <audio controls src={session.audioURL} style={{ width: '100%' }}></audio>
                        </ReportCard>
                    )}

                    <ReportCard title="Full Transcript" color="gray">
                        <p style={{ fontStyle: 'italic' }}>"{session.transcript}"</p>
                    </ReportCard>
                    ...
                    <ReportCard title="Feedback Summary" color="blue">
                    ...
                    <div className="report-grid-2">
                        <ReportCard title="Areas for Improvement" color="green">
                            <ul style={{ listStyleType: 'disc', listStylePosition: 'inside' }}>
                                {session.improvements.map((item, index) => (
                                    <li key={index}>
                                        {item.point ? <strong>{item.point}:</strong> : null} {item.explanation || item}
                                    </li>
                                ))}
                            </ul>
                        </ReportCard>
                        <ReportCard title="Potential Mistakes" color="orange">
                            <ul style={{ listStyleType: 'disc', listStylePosition: 'inside' }}>
                                {session.mistakes.filter(m => m.point || m).map((item, index) => (
                                    <li key={index}>
                                        {item.point ? <strong>{item.point}:</strong> : null} {item.explanation || item}
                                    </li>
                                ))}
